...
pub fn segment(rows: &[Row], s: &Settings, key: &str) -> Option<String> {
    ...
    let windows: Vec<String> = (0..3)
        ...
        .filter_map(|w| {
            ...
            Some(match s.tab_dots {
                0 => format!("{} {}%", LABELS[w], r.pct),
                dots => {
...
mod tests {
    ...
    fn numbers_only_and_windows_follow_the_settings() {
        ...
        let s = Settings { tab_agents: [true; 3], tab_windows: [false, true, true], tab_dots: 0, ..Settings::default() };
        assert_eq!(segment(&rows, &s, "claude").as_deref(), Some("Claude wk 34%"));
        let thick = Settings { tab_dots: 3, tab_windows: [true, false, false], ..s };
        ...
        let all = Settings { tab_windows: [true; 3], tab_dots: 3, ..thick };
